...
hold on
plot(Trigger(1:1:19,3),ee_all(Trigger_count(1:1:19,3),3),'o','color',[0 0.4470 0.7410],'LineWidth',1,...
    'LineWidth',1,'MarkerSize',4);
xlabel('$t$','Interpreter','Latex');%ylabel(strcat('$f_3$'),'Interpreter','Latex');
set(gca,'XTick',[2,4,6,8,10]) 
ylim([0,6.5]);
legend({strcat('$\|K{e}_{3}\|^2$'), ...
    strcat(['$\frac{\omega}{4l_{33}}\sum_{j\in \mathcal{N}_{3}}a_{3j}\|K(\hat{x}_{3}-\hat{x}_{j})\|^{2}+\beta\sigma_3(t)$']),'${t}^3_k$'},'NumColumns',3,'Interpreter','Latex','FontSize',10')
